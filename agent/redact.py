...
def redact_sensitive_text(text: str, *, force: bool = False) -> str:
    ...
    Disabled by default -- enable via security.redact_secrets: true in
    config.yaml. Pass ``force=True`` for security boundaries that must never
    expose secrets regardless of the global display setting.
    """
    if text is None:
        return None
