...
def _build_compact_banner_for_test(version_label: str) -> str:
    """Build the compact banner with globals patched on the function under test."""
    banner_globals = _build_compact_banner.__globals__
    with patch.object(
        banner_globals["shutil"],
        "get_terminal_size",
        return_value=SimpleNamespace(columns=90),
    ), patch.dict(
        banner_globals,
        {"format_banner_version_label": lambda: version_label},
    ):
        return _build_compact_banner()
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_hermes_branding(self):
        ...
        banner = _build_compact_banner_for_test("Hermes Agent v0.1.0 (test)")
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        banner = _build_compact_banner_for_test(
